...
export default function DocumentFormClient({
  ...
  useEffect(() => {
    ...
    fetch("/api/academic-years")
      ...
      .then((d) => {
        ...
        // Danh sách đã sắp xếp giảm dần theo tên - mặc định chọn năm học gần nhất cho tiện,
        // người dùng vẫn có thể đổi lại trước khi lưu.
        if (years.length > 0) {
          setForm((f) => (f.academicYearId ? f : { ...f, academicYearId: String(years[0].id) }));
        }
      });
  }, []);
  ...
  return (
    <form onSubmit={handleSubmit} className="card space-y-5 p-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="field-label">Năm học lưu trữ</label>
          {academicYears.length > 0 ? (
            ...
            >
              <option value="">-- Chưa gắn năm học --</option>
              {academicYears.map((y) => (
            ...
            <p className="field-input flex items-center bg-slate-50 text-slate-400">
              Chưa có năm học nào - vào Quản trị hệ thống → Năm học để thêm.
            </p>
          )}
        </div>
      </div>
